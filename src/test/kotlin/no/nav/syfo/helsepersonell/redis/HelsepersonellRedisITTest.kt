...
class HelsepersonellRedisITTest :
    FunSpec({
        ...
        val jedisPool =
            JedisPool(JedisConfig(), redisContainer.host, redisContainer.getMappedPort(6379))
        ...
        beforeTest {
            ...
            jedis.auth("secret")
            jedis.keys("**").forEach { jedis.del(it) }
        ...
        context("Test redis") {
            test("Should cache behandler in redis") {
                ...
                cachedBehandlerFnr shouldBeEqualTo
                    JedisBehandlerModel(
                        timestamp = cachedBehandlerFnr!!.timestamp,
                        behandler = behandler
                    )
